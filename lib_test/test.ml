...
struct
  ...
  let test_of_string_linux () =
    let host = Sys_stat_host.Linux.V4_1_12.Musl.v1_1_12 in
    let of_string = Sys_stat.Mode.of_string ~host:host.Sys_stat.Host.mode in
    ListLabels.iter string_test_cases
      ~f:(fun (expected_kind, expected_perm, string) ->
          Printf.ksprintf Alcotest.(check bool)
            "%s parses as (%s, 0o%o)"
            string (Sys_stat.File_kind.to_string expected_kind) expected_perm
            true
            (of_string string = (expected_kind, expected_perm)))
